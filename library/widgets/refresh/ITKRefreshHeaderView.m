...
// 刷新状态文字提示
#define kRefreshStateIdleText           NSLocalizedString(@"itk_header_refresh_idle", nil)
#define kRefreshStatePullingText        NSLocalizedString(@"itk_header_refresh_pulling",nil)
#define kRefreshStateRefreshingText     NSLocalizedString(@"itk_header_refresh_refreshing",nil)

static const CGFloat kRefreshHeaderHeight = 44.0;
...
// 状态文字字体颜色
static NSString* const kRereshStateFontColor = @"#666";
...
- (UILabel*)stateLabel
{
    if (!_stateLabel) {
        ...
        stateLabel.textColor = [UIColor itk_colorWithHexString:kRereshStateFontColor];
...
- (instancetype)init
{
    _layoutWidth = SCREEN_WIDTH;
    self = [self initWithFrame:CGRectMake(0, 0,_layoutWidth, kRefreshHeaderHeight)];
    return self;
}
...
- (void)layoutSubviews
{
    ...
    // 1.状态标签
    _stateLabel.frame = CGRectMake(0, 0, _layoutWidth, stateH);
    _stateLabel.textAlignment = NSTextAlignmentCenter;
    
    // 2. 头像图标位置
    CGFloat arrowX = _layoutWidth * 0.5 - 44;
    self.imageIcon.center = CGPointMake(arrowX, self.itk_height * 0.5);
    
    self.activityView.center = self.imageIcon.center;
}
...
- (UIImageView*)imageIcon
{
    if (!_imageIcon) {
        // 头像
        UIImageView* iconImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon-refresh-top"]];
        
        UIActivityIndicatorView* loadingView = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
        _activityView = loadingView;
...
- (void) willPullingAnim{
    [UIView animateWithDuration:0.3f animations:^{
        self.imageIcon.transform = CGAffineTransformMakeRotation(M_PI);
    }];
}
...
- (void)setState:(ITKRefreshHeaderState)state
{
    ...
    case ITKRefreshStateIdle: {
        if (oldState == ITKRefreshStateRefreshing) {
            ...
            __weak ITKRefreshHeaderView *owner = self;
            // 恢复inset和offset
            [UIView animateWithDuration:kRefreshSlowAnimationDuration
                             ...
                             animations:^{
                                 // 修复top值不断累加
                                 owner.scrollView.itk_insetTop -= self.itk_height;
                             }
                             completion:nil];
        } else {
        ...
        case ITKRefreshStatePulling:{
            [self willPullingAnim];
            break;
        }
    case ITKRefreshStateRefreshing: {
        ...
        [UIView animateWithDuration:kRefreshFastAnimationDuration
            ...
            animations:^{
                // 增加滚动区域
                CGFloat top = owner.scrollViewOriginalInset.top + self.itk_height;
                owner.scrollView.itk_insetTop = top;
                // 设置滚动位置
                owner.scrollView.itk_offsetY = -top;
            }
...
- (UIEdgeInsets)scrollViewOriginalInset{
...
- (void)willMoveToSuperview:(UIView*)newSuperview
{
    ...
    if (newSuperview) { // 新的父控件
        ...
        // 设置宽度
        //self.frame = CGRectMake(0, self.itk_y, newSuperview, kRefreshHeaderHeight);
        
        self.itk_y = 0;
        self.itk_height = kRefreshHeaderHeight;
